...
const getDataExamples = function getDataExamples () {
  ...
  Object.keys(dataColumns).map(async (key) => {
    const { data } = await Axios.get(`http://localhost:8121/${key}`, objParams)
...
module.exports = {
  ...
  'Load All The Pages' (browser) {
    ...
    for (let index = 0; index < objList.length; index += 1) {
      ...
      browser.waitForElementPresent(`div[id=listitem-${objList[index].replace(/_/gu, '-')}]`)
      browser.click(`div[id=listitem-${objList[index].replace(/_/gu, '-')}]`)
      browser.pause(pauseTime)
      browser.assert.containsText('#root', obj[dataColumns[objList[index]]])
    }
